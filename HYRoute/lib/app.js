...
app.use(function(req, res, next) {
	...
	res.writeHead(301, {"Location" : "/www/" + (platform ?"welcome_app.html":"welcome_pc.html")});
	res.end();
});
app.use(function(req, res, next) {
...
app.use(route);
app.use(function(req, res, next) {
	if (!hy.use(req, res, next))
		return;

	if (req.url.indexOf('/native') != -1) {
		global.webapi = "https://soa-app.j1.com/";
	} else {
		global.webapi = "https://soa-app.j1.com/";
	}
	...
	if (req.url.indexOf('/newmall') != -1) {
		global.webapi = "http://h5.api.j1.com/";
	} else {
		global.webapi = "https://soa-app.j1.com/";
	}
